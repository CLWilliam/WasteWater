...
package ASM2d "Component models for the Activated Sludge Model No.2d"
  ...
  model deni "ASM2d denitrification tank"
    ...
    parameter Modelica.Units.SI.Volume V = 1000 "Volume of denitrification tank";
...
Parameters:
  ...
  model nitri "ASM2d nitrification tank"
    ...
    parameter Modelica.Units.SI.Volume V = 1000 "Volume of nitrification tank";
    parameter Real alpha = 0.7 "Oxygen transfer factor";
    parameter Modelica.Units.SI.Length de = 4.5 "depth of aeration";
...
Parameters:
  ...
  model precipitation "Phosphorus precipitation tank"
    ...
    extends Interfaces.ASM2dbase;
    parameter Modelica.Units.SI.Volume V = 50 "Volume of precipitation tank";
...
Parameter:
  ...
  model pump "ASM2d wastewater pump"
    ...
    Interfaces.WWFlowAsm2din In annotation(
      Placement(transformation(extent = {{-110, -43}, {-90, -23}})));
    Interfaces.WWFlowAsm2dout Out annotation(
      Placement(transformation(extent = {{90, 18}, {110, 38}})));
    Modelica.Blocks.Interfaces.RealInput u annotation(
      Placement(transformation(extent = {{-98, 15}, {-78, 35}})));
  equation
    H = 0.5*(-Q_min + Q_max) + u*0.5*(-Q_min + Q_max) + Q_min;
...
Parameter:
  ...
  model FlowSource "Flowsource"
    ...
    Interfaces.WWFlowAsm2dout Out annotation(
      Placement(transformation(extent = {{88, -80}, {108, -60}})));
    Modelica.Blocks.Interfaces.RealInput data annotation(
...
The dimension of InPort is 1.
  ...
  model WWSource "Wastewater source"
    ...
    Interfaces.WWFlowAsm2dout Out annotation(
      Placement(transformation(extent = {{88, -80}, {108, -60}})));
    Modelica.Blocks.Interfaces.RealInput data[19] annotation(
      Placement(transformation(extent = {{-100, -10}, {-80, 10}})));
  equation
    Out.Q = -data[1];
    Out.So = data[2];
    ...
    Out.Xpha = data[16];
    Out.Xa = data[17];
    Out.Xtss = i_TSS_Xi*Out.Xi + i_TSS_Xs*Out.Xs + i_TSS_BM*(Out.Xh + Out.Xpao + Out.Xa) + 3.23*Out.Xpp + 0.6*Out.Xpha + Out.Xmeoh + Out.Xmep;
    Out.Xmeoh = data[18];
    Out.Xmep = data[19];
    annotation(
...
Storage or further sludge treatment is not jet considered."));
  ...
  model ControlledDivider2 "Controlled flow divider"
    ...
    Interfaces.WWFlowAsm2dout Out1 annotation(
      Placement(transformation(extent = {{90, 15}, {110, 35}})));
    Interfaces.WWFlowAsm2dout Out2 annotation(
...
The concentrations of the outport-flows are equal to the concentration at inport."));
  ...
  equation
    ...
    Out2.Xmep = In.Xmep;
    annotation(
      Documentation(info = "This component divides one ASM2d wastewater flow into two ASM2d wastewater flows."));
  end divider2;

  model mixer2 "Mixer of two ASM2d characterised flows"
    extends WasteWater.Icons.mixer2;
    Interfaces.WWFlowAsm2din In1 annotation(
      Placement(transformation(extent = {{-110, 15}, {-90, 35}})));
    Interfaces.WWFlowAsm2din In2 annotation(
      Placement(transformation(extent = {{-110, -25}, {-90, -5}})));
    Interfaces.WWFlowAsm2dout Out annotation(
      Placement(transformation(extent = {{90, -6}, {110, 14}})));
  equation
    In1.Q + In2.Q + Out.Q = 0;
    ...
    annotation(
      Documentation(info = "This component mixes two flows of wastewater (ASM2d) of different concentration and different amount."));
  end mixer2;

  model mixer3 "Mixer of 3 ASM2d characterised flows"
    extends WasteWater.Icons.mixer3;
    Interfaces.WWFlowAsm2din In1 annotation(
      Placement(transformation(extent = {{-110, 25}, {-90, 45}})));
    Interfaces.WWFlowAsm2din In2 annotation(
      Placement(transformation(extent = {{-110, -15}, {-90, 5}})));
    Interfaces.WWFlowAsm2din In3 annotation(
      Placement(transformation(extent = {{-110, -55}, {-90, -35}})));
    Interfaces.WWFlowAsm2dout Out annotation(
      Placement(transformation(extent = {{90, -14}, {110, 6}})));
  equation
    ...
    annotation(
      Documentation(info = "This component mixes 3 flows of wastewater (ASM2d) of different concentration and different amount."));
  end mixer3;
  ...
  model sensor_COD "Ideal sensor to measure chemical oxygen demand (COD)"
    ...
    Modelica.Blocks.Interfaces.RealOutput COD annotation(
...
further processed with blocks of the Modelica.Blocks library).
      "),
      Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}, grid = {2, 2}), graphics = {Ellipse(extent = {{-50, 50}, {50, -50}}, lineColor = {0, 0, 0}, lineThickness = 0.5, fillColor = {223, 223, 159}, fillPattern = FillPattern.Solid), Text(extent = {{-80, 100}, {80, 60}}, textString = "%name"), Line(points = {{0, 50}, {0, 38}}, thickness = 0.5), Line(points = {{-50, 0}, {38, 0}}, thickness = 0.5), Line(points = {{50, 0}, {38, 0}}, thickness = 0.5), Line(points = {{-36, 34}, {-28, 26}}, thickness = 0.5), Line(points = {{34, 36}, {26, 28}}, thickness = 0.5), Line(points = {{0, 0}, {26, 28}}, thickness = 0.5), Polygon(points = {{30, 32}, {10, 24}, {24, 12}, {30, 32}}, lineColor = {0, 0, 0}, fillColor = {0, 0, 0}, fillPattern = FillPattern.Solid), Text(extent = {{-36, -10}, {36, -32}}, textString = "O2"), Line(points = {{0, -50}, {0, -90}}, thickness = 0.5), Line(points = {{50, 0}, {88, 0}})}));
  end sensor_O2;
  ...
  model sensor_PO "Ideal sensor to measure dissolved phosphorus"
    ...
    Interfaces.WWFlowAsm2din In annotation(
      Placement(transformation(extent = {{-10, -110}, {10, -90}})));
    Modelica.Blocks.Interfaces.RealOutput Spo annotation(
      Placement(transformation(extent = {{88, -10}, {108, 10}})));
  equation
    In.Q = 0;
    Spo = In.Spo;
...
further processed with blocks of the Modelica.Blocks library).
  ...
  model sensor_Q "Ideal sensor to measure the flow rate of an ASM2d wastewater stream"
...
the Modelica.Blocks library).
  ...
  model sensor_TKN "Ideal TKN and total nitrogen sensor"
    ...
    Interfaces.WWFlowAsm2din In annotation(
      Placement(transformation(extent = {{-10, -110}, {10, -90}})));
    Modelica.Blocks.Interfaces.RealOutput TKN[2] annotation(
      Placement(transformation(extent = {{88, -10}, {108, 10}})));
  equation
    In.Q = 0.0;
    TKN[1] = In.Snh + i_N_Si*In.Si + i_N_Sf*In.Sf + i_N_Xi*In.Xi + i_N_Xs*In.Xs + i_N_BM*(In.Xh + In.Xpao + In.Xa);
    TKN[2] = TKN[1] + In.Sno;
    annotation(
...
signal[2] - N_total
  ...
  model sensor_TP "Ideal sensor to measure the total phosphorus concentration in ASM2d wastewater"
    extends WasteWater.Icons.sensor_TP;
    extends Interfaces.conversion_factors;
    Interfaces.WWFlowAsm2din In annotation(
      Placement(transformation(extent = {{-10, -110}, {10, -90}})));
    Modelica.Blocks.Interfaces.RealOutput TP annotation(
      Placement(transformation(extent = {{88, -10}, {108, 10}})));
  equation
    In.Q = 0.0;
    TP = In.Spo + In.Xpp + i_P_Sf*In.Sf + i_P_Si*In.Si + i_P_Xi*In.Xi + i_P_Xs*In.Xs + i_P_BM*(In.Xh + In.Xa + In.Xpao) + In.Xmep/4.87;
    annotation(
      Documentation(info = "This component measures the total phosphorus concentration [g/m3]
of ASM2d wastewater and provides the result as output signal (to be
further processed with blocks of the Modelica.Blocks library).
      "));
  end sensor_TP;

  model sensor_TSS "Ideal sensor to measure total suspended solids concentration (ASM2d)"
    extends WasteWater.Icons.sensor_TSS;
    Interfaces.WWFlowAsm2din In annotation(
      Placement(transformation(extent = {{-10, -110}, {10, -90}})));
    Modelica.Blocks.Interfaces.RealOutput TSS annotation(
      Placement(transformation(extent = {{88, -10}, {108, 10}})));
  equation
    In.Q = 0;
    TSS = In.Xtss;
