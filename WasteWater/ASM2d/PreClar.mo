...
package PreClar "Primary clarifier modelling based on ASM2d"
  ...
  model preclar1 "Dynamic ASM2d Primary Clarifier Model"
    ...
    ASM2d.Interfaces.WWFlowAsm2dout MeasurePort annotation(
...
// total COD in the influent
    CODout = Out.Si + Out.Sf + Out.Sa + Out.Xi + Out.Xs + Out.Xh + Out.Xpao + Out.Xpha + Out.Xa;
...
// so the amount of primary sludge removed can not be calculated
    ...
    Out.Xpha = (1 - n_X)*Xpha;
    Out.Xa = (1 - n_X)*Xa;
    Out.Xtss = i_TSS_Xi*Out.Xi + i_TSS_Xs*Out.Xs + i_TSS_BM*(Out.Xh + Out.Xpao + Out.Xa) + 0.6*Out.Xpha + Out.Xmeoh + Out.Xmep + 3.23*Out.Xpp;
    Out.Xmeoh = 0.0;
    Out.Xmep = 0.0;
    MeasurePort.So = So;
    MeasurePort.Sf = Sf;
...
// 0.9*Out.Xa + 0.6*Out.Xpha;
    MeasurePort.Xtss = i_TSS_Xi*Out.Xi + i_TSS_Xs*Out.Xs + i_TSS_BM*(Out.Xh + Out.Xpao + Out.Xa) + 0.6*Out.Xpha + Out.Xmeoh + Out.Xmep + 3.23*Out.Xpp;
...
// aXpha*CODin;
    In.Xa = aXa*CODin;
    In.Xtss = i_TSS_Xi*In.Xi + i_TSS_Xs*In.Xs + i_TSS_BM*(In.Xh + In.Xpao + In.Xa) + 0.6*In.Xpha + In.Xmeoh + In.Xmep + 3.23*In.Xpp;
    In.Xmeoh = 0.0;
    In.Xmep = 0.0;
// Outputs
    Out.So = So;
