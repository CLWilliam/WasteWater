...
Parameters:
  ...
  model blower "Blower for the aeration of the nitrification tanks"
    ...
    Modelica.Blocks.Interfaces.RealInput u annotation(
      Placement(transformation(origin = {99, -30}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  equation
...
Parameter:
  ...
  model pump "ASM3 wastewater pump"
    ...
    Interfaces.WWFlowAsm3out Out annotation(
      Placement(transformation(extent = {{90, 19}, {110, 39}})));
    Modelica.Blocks.Interfaces.RealInput u annotation(
...
The dimension of InPort is 1.
  ...
  model WWSource "Wastewater source"
    ...
    Interfaces.WWFlowAsm3out Out annotation(
      Placement(transformation(extent = {{88, -80}, {108, -60}})));
    Modelica.Blocks.Interfaces.RealInput data[13] annotation(
      Placement(transformation(extent = {{-100, -10}, {-80, 10}})));
  equation
    Out.Q = -data[1];
    Out.So = data[2];
...
Storage or further sludge treatment is not jet considered."));
  ...
  model ControlledDivider2 "Controlled flow divider"
    ...
    Interfaces.WWFlowAsm3in In annotation(
      Placement(transformation(extent = {{-111, -6}, {-91, 14}})));
    Interfaces.WWFlowAsm3out Out1 annotation(
      Placement(transformation(extent = {{90, 20}, {110, 40}})));
    Interfaces.WWFlowAsm3out Out2 annotation(
      Placement(transformation(extent = {{90, -28}, {110, -8}})));
    Modelica.Blocks.Interfaces.RealInput u annotation(
      layer = "icon",
...
The concentrations of the outport-flows are equal to the concentration at inport."));
  ...
  model divider2 "Flowdivider"
    ...
    Interfaces.WWFlowAsm3out Out1 annotation(
      Placement(transformation(extent = {{89, 20}, {109, 40}})));
    Interfaces.WWFlowAsm3out Out2 annotation(
      Placement(transformation(extent = {{90, -28}, {110, -8}})));
  equation
    ...
    Out2.Xss = In.Xss;
    annotation(
      Documentation(info = "This component divides one ASM3 wastewater flow into two ASM3 wastewater flows."));
  end divider2;

  model mixer2 "Mixer of two ASM3 characterised flows"
    extends WasteWater.Icons.mixer2;
    Interfaces.WWFlowAsm3in In1 annotation(
      Placement(transformation(extent = {{-111, 20}, {-91, 40}})));
    Interfaces.WWFlowAsm3in In2 annotation(
      Placement(transformation(extent = {{-111, -30}, {-91, -10}})));
    Interfaces.WWFlowAsm3out Out annotation(
      Placement(transformation(extent = {{90, -5}, {110, 15}})));
  equation
    In1.Q + In2.Q + Out.Q = 0;
    ...
    annotation(
      Documentation(info = "This component mixes two flows of wastewater (ASM3) of different concentration and different amount."));
  end mixer2;
  ...
  model mixer3 "Mixer of 3 ASM3 characterised flows"
    ...
    Interfaces.WWFlowAsm3in In1 annotation(
      Placement(transformation(extent = {{-110, 29}, {-90, 49}})));
    Interfaces.WWFlowAsm3in In2 annotation(
      Placement(transformation(extent = {{-111, -14}, {-91, 6}})));
    Interfaces.WWFlowAsm3in In3 annotation(
      Placement(transformation(extent = {{-109, -59}, {-89, -39}})));
    Interfaces.WWFlowAsm3out Out annotation(
      Placement(transformation(extent = {{90, -13}, {110, 7}})));
  equation
    ...
    annotation(
      Documentation(info = "This component mixes 3 flows of wastewater (ASM3) of different concentration and different amount."));
  end mixer3;
...
the Modelica.Blocks library).
  ...
  equation
    In.Q = 0.0;
    TKN[1] = i_N_Si*In.Si + i_N_Ss*In.Ss + In.Snh + i_N_Xi*In.Xi + i_N_Xs*In.Xs + i_N_BM*(In.Xh + In.Xa);
    TKN[2] = TKN[1] + In.Snox;
    annotation(
...
signal[2] - N_total
  ...
  model sensor_TSS "Ideal sensor to measure total suspended solids concentration (ASM3)"
